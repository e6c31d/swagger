...
export function getTypeReferenceAsString(
  ...
  if (type.isClass()) {
    const importedNameInSourceFile = getImportedNameInSourceFile(
      type,
      typeChecker,
      sourceFile
    );
    if (importedNameInSourceFile) {
      return importedNameInSourceFile;
    }
...
function getImportedNameInSourceFile(
  type: ts.Type,
  typeChecker: ts.TypeChecker,
  sourceFile: ts.SourceFile | undefined
): string | undefined {
  if (!sourceFile || sourceFile.impliedNodeFormat !== ts.ModuleKind.ESNext) {
    return undefined;
  }

  // WARNING: Ineffecient O(n^2) complexity
  const sourceFileSymbols = typeChecker.getSymbolsInScope(
    sourceFile,
    ts.SymbolFlags.Class | ts.SymbolFlags.Alias
  );
  for (const symbol of sourceFileSymbols) {
    if (symbolIsTypeDeclaration(symbol, type, typeChecker)) {
...
function symbolIsTypeDeclaration(
  symbol: ts.Symbol,
  type: ts.Type,
  typeChecker: ts.TypeChecker
): boolean {
  for (const declaration of symbol.getDeclarations()) {
    if (declaration === type.symbol.getDeclarations()[0]) {
      // Class is declared in the same file
      return true;
    }
    ...
    if (
      ...
      // import has "type" modifier, so it won't be available at runtime
      // example: import type { MyType } from ...
      return false;
    }
  }

  return type === typeChecker.getDeclaredTypeOfSymbol(symbol);
}
